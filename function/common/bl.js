...
async function bl(interaction, secret, trans, user, status, word) {
  ...
  const invalid_status = trans.strings.find(
    (it) => it.name === "invalid_status"
  ).trans;
  const already_banned = trans.strings.find(
    (it) => it.name === "already_banned"
  ).trans;
  const banned = trans.strings.find((it) => it.name === "banned").trans;
  const no_word_banned = trans.strings.find(
    (it) => it.name === "no_word_banned"
  ).trans;
  ...
  const list_of_word_banned = trans.strings.find(
    (it) => it.name === "list_of_word_banned"
  ).trans;
  const sending_list_of_word_banned = trans.strings.find(
    (it) => it.name === "sending_list_of_word_banned"
  ).trans;
  ...
  if (status === "ban") {
    ...
    interaction.reply(
      `\`\`\`${word}\`\`\` ${banned} <a:isis:963826754328330300>`
    );
