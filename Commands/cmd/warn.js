    ...
    try {
      ...
      if (message.author.bot)
        return message.reply(
          `${bot} <:pekora_whatwrongwithyou:976146270743855217>`
        );
      ...
      const no_member = trans.strings.find(
        (it) => it.name === "no_member"
      ).trans;
      if (!member)
      ...
      if (member.user.id === secret.botid)
        return message.reply(
          `${warn_yourself} <a:z_sui_eating:976448366781267998>`
        );
      // Check for reason to timeout
      // Use IDK if not provided :))
      const idk = trans.strings.find((it) => it.name === "idk").trans;
      const reason =
        message.content.split(">")[1] || `${idk} <:0V0:970325975810334750>`;
      // I can't timeout Admin :/
      const warn_admin = trans.strings.find(
        (it) => it.name === "warn_admin"
      ).trans;
      ...
      message.channel
        .send(`<@${member}>${stfu}5${min}<:bananaV3:958346989597241344>`)
        .catch(console.log);
      // If there is a warning channel
      // Working on a command to set it. For now, was hardcoded and for private use only.
      let channel_id =
        message.guild.id === secret.bot_testing_grp
          ? secret.bot_testing_grp_warn
          : message.guild.id === secret.bot_grp1
          ? secret.bot_grp1_warn
          : null;
