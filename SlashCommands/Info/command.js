        ...
        try {
            ...
            if (status === "enable") {
                ...
                if (!JSON.stringify(now)?.includes(interaction.channel.id)) {
                    const alr_enabled = trans.strings.find(it => it.name === "alr_enabled").trans;
                ...
                if (JSON.stringify(now)?.includes(interaction.channel.id)) {
                    const alr_disabled = trans.strings.find(it => it.name === "alr_disabled").trans;
                    return interaction.reply({ content: alr_disabled, ephemeral: true });
                }

                await db.push("no_commands", interaction.channel.id);
                const disabled = trans.strings.find(it => it.name === "disabled").trans;
