...
module.exports = {
  ...
  async execute(client, interaction, args, secret, trans) {
    ...
    try {
      ...
      let emoji = true;
      const reactionid = reaction.includes("<a:")
        ? reaction.split("<a:")[1].split(":")[1].split(">")[0]
        : reaction.split("<:")[1].split(":")[1].split(">")[0];
      ...
      if (status === "add") {
        const no_access = trans.strings.find(
          (it) => it.name === "no_access"
        ).trans;
        if (
          !emoji &&
          !message.reactions.cache.find((v) => v._emoji.id == reactionid)
        )
          return interaction.reply({
            content: `${no_access} ${reaction}`,
            ephemeral: true,
          });
        const sending = trans.strings.find((it) => it.name === "sending").trans;
        await interaction.reply({
          content: `${sending} ${reaction}`,
          ephemeral: true,
        });
        message.react(reaction);
      } else {
        const react_not_found = trans.strings.find(
          (it) => it.name === "react_not_found"
        ).trans;
        if (!message.reactions.cache.find((v) => v._emoji.id == reactionid))
          return interaction.reply({
            content: `${react_not_found} ${reaction}`,
            ephemeral: true,
          });
        const removing = trans.strings.find(
          (it) => it.name === "removing"
        ).trans;
        await interaction.reply({
