...
module.exports = {
  ...
  async execute(client, interaction, args, secret, trans) {
    ...
    const using_default = trans.strings.find(
      (it) => it.name === "using_default"
    ).trans;
    const server_default = trans.strings.find(
      (it) => it.name === "server_default"
    ).trans;
    const not_set = trans.strings.find((it) => it.name === "not_set").trans;
    const info = trans.strings.find((it) => it.name === "info").trans;
    const group_lang = trans.strings.find(
      (it) => it.name === "group_lang"
    ).trans;
    const cur_prefix = trans.strings.find(
      (it) => it.name === "cur_prefix"
    ).trans;
    const svr_wel_ch = trans.strings.find(
      (it) => it.name === "svr_wel_ch"
    ).trans;
    let lanh = lang_c
      ? `\`\`${lang_c}(${
          lang.filter((it) => it.code === lang_c)[0]?.name
        })\`\` \n${set_by}: <@${lang_author}> \n${at}: <t:${lang_time}>`
      : `\`\`${message.guild.preferredLocale}(${
          lang.filter((it) => it.code === message.guild.preferredLocale)[0]
            ?.name
        })\`\` (${server_default})`;
