...
module.exports = {
  ...
  async execute(client, interaction, args, secret, trans) {
    ...
    async function getSystemInfo() {
      ...
      let installedPackagesCmd =
        os.platform() === "win32"
          ? "wmic product get name"
          : "dpkg --get-selections";
      let hardDiskName = await exec(hardDiskCmd);
      let installedPackages = await exec(
        `${
          os.platform() === "win32"
            ? 'powershell "Get-ItemProperty HKLM:\\Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\* | Select-Object DisplayName | Measure-Object | % { $_.Count }"'
            : "dpkg --get-selections | wc -l"
        }`
      );
      return `Device Name: ${os.hostname()},\nKernel Version: ${os.release()},\nHardDisk Name: ${
        hardDiskName.stdout.split("\n")[1].split("  \r")[0]
    ...
    async function systemStatus() {
      let str = `System Status\nOS: ${os.type()} ${os.release()}\nCPU: ${
        os.cpus()[0].model
      } @ ${os.cpus()[0].speed} MHz (${os.cpus().length} cores)\nRAM: Total: ${(
        os.totalmem() /
        1024 ** 3
      ).toFixed(2)} GB, Used: ${(
      ...
      try {
        ...
        str += `Storage (${os.platform() === "win32" ? "C:" : "/"}): Total: ${
          storage.total
        } GB, Used: ${storage.used} GB, Free: ${storage.free} GB\n`;
